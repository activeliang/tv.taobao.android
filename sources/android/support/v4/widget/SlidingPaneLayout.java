        ...
        for (int i = 0; i < childCount; i++) {
            ...
            if (child.getVisibility() == 8) {
                ...
                if (lp.width == -2) {
                    ...
                    childWidthSpec = View.MeasureSpec.makeMeasureSpec(widthAvailable - horizontalMargin, UCCore.VERIFY_POLICY_QUICK);
                } else {
                    childWidthSpec = View.MeasureSpec.makeMeasureSpec(lp.width, UCCore.VERIFY_POLICY_QUICK);
                }
                if (lp.height == -2) {
                    childHeightSpec3 = View.MeasureSpec.makeMeasureSpec(maxLayoutHeight, Integer.MIN_VALUE);
                } else if (lp.height == -1) {
                    childHeightSpec3 = View.MeasureSpec.makeMeasureSpec(maxLayoutHeight, UCCore.VERIFY_POLICY_QUICK);
                } else {
                    childHeightSpec3 = View.MeasureSpec.makeMeasureSpec(lp.height, UCCore.VERIFY_POLICY_QUICK);
                }
                ...
                if (child2.getVisibility() != 8) {
                    ...
                    if (child2.getVisibility() != 8) {
                        ...
                        if (!canSlide || child2 == this.mSlideableView) {
                            if (lp2.weight > 0.0f) {
                                if (lp2.width != 0) {
                                    childHeightSpec = View.MeasureSpec.makeMeasureSpec(child2.getMeasuredHeight(), UCCore.VERIFY_POLICY_QUICK);
                                    ...
                                    childHeightSpec = View.MeasureSpec.makeMeasureSpec(maxLayoutHeight, UCCore.VERIFY_POLICY_QUICK);
                                } else {
                                    childHeightSpec = View.MeasureSpec.makeMeasureSpec(lp2.height, UCCore.VERIFY_POLICY_QUICK);
                                }
                                if (canSlide) {
                                    int newWidth = widthAvailable - (lp2.leftMargin + lp2.rightMargin);
                                    int childWidthSpec2 = View.MeasureSpec.makeMeasureSpec(newWidth, UCCore.VERIFY_POLICY_QUICK);
                                    if (measuredWidth != newWidth) {
                                        child2.measure(childWidthSpec2, childHeightSpec);
                                    }
                                } else {
                                    child2.measure(View.MeasureSpec.makeMeasureSpec(measuredWidth + ((int) ((lp2.weight * ((float) Math.max(0, widthRemaining))) / weightSum)), UCCore.VERIFY_POLICY_QUICK), childHeightSpec);
                                }
                            }
                        } else if (lp2.width < 0 && (measuredWidth > fixedPanelWidthLimit || lp2.weight > 0.0f)) {
                            if (!skippedFirstPass) {
                                childHeightSpec2 = View.MeasureSpec.makeMeasureSpec(child2.getMeasuredHeight(), UCCore.VERIFY_POLICY_QUICK);
                            } else if (lp2.height == -2) {
                                childHeightSpec2 = View.MeasureSpec.makeMeasureSpec(maxLayoutHeight, Integer.MIN_VALUE);
                            } else if (lp2.height == -1) {
                                childHeightSpec2 = View.MeasureSpec.makeMeasureSpec(maxLayoutHeight, UCCore.VERIFY_POLICY_QUICK);
                            } else {
                                childHeightSpec2 = View.MeasureSpec.makeMeasureSpec(lp2.height, UCCore.VERIFY_POLICY_QUICK);
                            }
                            child2.measure(View.MeasureSpec.makeMeasureSpec(fixedPanelWidthLimit, UCCore.VERIFY_POLICY_QUICK), childHeightSpec2);
                        }
                    }
                }
            }
