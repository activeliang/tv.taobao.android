...
public final class LinkifyCompat {
    ...
    public static final boolean addLinks(@NonNull Spannable text, int mask) {
        ...
        if ((mask & 2) != 0) {
            gatherLinks(links, text, PatternsCompat.AUTOLINK_EMAIL_ADDRESS, new String[]{WVUCWebViewClient.SCHEME_MAILTO}, (Linkify.MatchFilter) null, (Linkify.TransformFilter) null);
        }
        if ((mask & 8) != 0) {
        ...
        while (true) {
            try {
                ...
                if (address != null && (start = string.indexOf(address)) >= 0) {
                    ...
                    base += end;
                    try {
                        spec.url = WVUCWebViewClient.SCHEME_GEO + URLEncoder.encode(address, "UTF-8");
                        links.add(spec);
