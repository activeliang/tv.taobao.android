    ...
    public void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        if (widthMode == Integer.MIN_VALUE) {
            ...
            if (!(tvw == null || tvw.type == 0)) {
                ...
                if (w > 0) {
                    widthMeasureSpec = View.MeasureSpec.makeMeasureSpec(Math.min(w - (this.mDecorPadding.left + this.mDecorPadding.right), View.MeasureSpec.getSize(widthMeasureSpec)), UCCore.VERIFY_POLICY_QUICK);
                    fixedWidth = true;
                }
            }
        }
        if (heightMode == Integer.MIN_VALUE) {
            ...
            if (!(tvh == null || tvh.type == 0)) {
                ...
                if (h > 0) {
                    heightMeasureSpec = View.MeasureSpec.makeMeasureSpec(Math.min(h - (this.mDecorPadding.top + this.mDecorPadding.bottom), View.MeasureSpec.getSize(heightMeasureSpec)), UCCore.VERIFY_POLICY_QUICK);
                }
            }
        }
        ...
        int widthMeasureSpec2 = View.MeasureSpec.makeMeasureSpec(width, UCCore.VERIFY_POLICY_QUICK);
        if (!fixedWidth && widthMode == Integer.MIN_VALUE) {
            TypedValue tv2 = isPortrait ? this.mMinWidthMinor : this.mMinWidthMajor;
            if (!(tv2 == null || tv2.type == 0)) {
                int min = 0;
                if (tv2.type == 5) {
                    min = (int) tv2.getDimension(metrics);
                } else if (tv2.type == 6) {
                    min = (int) tv2.getFraction((float) metrics.widthPixels, (float) metrics.widthPixels);
                }
                if (min > 0) {
                    min -= this.mDecorPadding.left + this.mDecorPadding.right;
                }
                if (width < min) {
                    widthMeasureSpec2 = View.MeasureSpec.makeMeasureSpec(min, UCCore.VERIFY_POLICY_QUICK);
                    measure = true;
                }
            }
        }
