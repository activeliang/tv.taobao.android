        ...
        try {
            ...
            if (header.getNumFrames() <= 0 || header.getStatus() != 0) {
                ...
                if (firstFrame == null) {
                    ...
                    gifDrawableResource = new GifDrawableResource(new GifDrawable(this.context, gifDecoder, UnitTransformation.get(), width, height, firstFrame));
                    if (Log.isLoggable(TAG, 2)) {
                        Log.v(TAG, "Decoded GIF from stream in " + LogTime.getElapsedMillis(startTime));
                    }
                }
            ...
            if (Log.isLoggable(TAG, 2)) {
                ...
                Log.v(TAG, str + LogTime.getElapsedMillis(startTime));
            }
        }
    }
