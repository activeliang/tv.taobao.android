...
public class AwtCodec implements ObjectSerializer, ObjectDeserializer {
    ...
    public void write(JSONSerializer serializer, Object object, Object fieldName, Type fieldType, int features) throws IOException {
        ...
        if (object instanceof Point) {
            ...
            out.writeFieldValue(writeClassName(out, Color.class, '{'), UploadQueueMgr.MSGTYPE_REALTIME, color.getRed());
    ...
    public <T> T deserialze(DefaultJSONParser parser, Type type, Object fieldName) {
        ...
        if (lexer.token() == 8) {
            ...
            lexer.nextToken();
            if (type == Point.class) {
                parseFont = parsePoint(parser, fieldName);
            } else if (type == Rectangle.class) {
                parseFont = parseRectangle(parser);
            } else if (type == Color.class) {
                parseFont = parseColor(parser);
            } else if (type == Font.class) {
                parseFont = parseFont(parser);
            } else {
                throw new JSONException("not support awt class : " + type);
            }
            ParseContext context = parser.getContext();
            parser.setContext(parseFont, fieldName);
        ...
        while (lexer.token() != 13) {
            if (lexer.token() == 4) {
                ...
                if (lexer.token() == 2) {
                    ...
                    lexer.nextToken();
                    if (key.equalsIgnoreCase(UploadQueueMgr.MSGTYPE_REALTIME)) {
                        r = val;
