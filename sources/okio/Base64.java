...
final class Base64 {
    ...
    private static String encode(byte[] in, byte[] map) {
        ...
        for (int i = 0; i < end; i += 3) {
            int index2 = index + 1;
            out[index] = map[(in[i] & OnReminderListener.RET_FULL) >> 2];
            int index3 = index2 + 1;
            out[index2] = map[((in[i] & 3) << 4) | ((in[i + 1] & OnReminderListener.RET_FULL) >> 4)];
            int index4 = index3 + 1;
            out[index3] = map[((in[i + 1] & 15) << 2) | ((in[i + 2] & OnReminderListener.RET_FULL) >> 6)];
            index = index4 + 1;
        ...
        switch (in.length % 3) {
            case 1:
                int index5 = index + 1;
                out[index] = map[(in[end] & OnReminderListener.RET_FULL) >> 2];
                int index6 = index5 + 1;
            ...
            case 2:
                ...
                out[index] = map[(in[end] & OnReminderListener.RET_FULL) >> 2];
                int index9 = index8 + 1;
                out[index8] = map[((in[end] & 3) << 4) | ((in[end + 1] & OnReminderListener.RET_FULL) >> 4)];
