...
public abstract class Observable<T> implements ObservableSource<T> {
    ...
    public static <T> Observable<T> concatEager(ObservableSource<? extends ObservableSource<? extends T>> sources, int maxConcurrency, int prefetch) {
        ObjectHelper.requireNonNull(Integer.valueOf(maxConcurrency), "maxConcurrency is null");
        ObjectHelper.requireNonNull(Integer.valueOf(prefetch), "prefetch is null");
    ...
    public static Observable<Long> intervalRange(long start, long count, long initialDelay, long period, TimeUnit unit, Scheduler scheduler) {
        if (count < 0) {
            ...
            if (start <= 0 || end >= 0) {
                ObjectHelper.requireNonNull(unit, "unit is null");
                ObjectHelper.requireNonNull(scheduler, "scheduler is null");
                return RxJavaPlugins.onAssembly(new ObservableIntervalRange(start, end, Math.max(0, initialDelay), Math.max(0, period), unit, scheduler));
            }
    ...
    public final Completable concatMapCompletable(Function<? super T, ? extends CompletableSource> mapper, int capacityHint) {
        ...
        return RxJavaPlugins.onAssembly((Completable) new ObservableConcatMapCompletable(this, mapper, capacityHint));
    }
